...
interface EditUserCmpProps {
  show: boolean;
  hide: () => void;
  toedit: any;
}

export default function EditUserCmp({ show, hide,toedit }: EditUserCmpProps) {
...
salary:'',
    ...
    const itemSubmit = (e:any) => {
       ...
       axios.put(`/api/users/${toedit?._id}`,data).then(res => {

        toast.success('Utilisateur Modifié avec succé', {
            position: "top-right",
            autoClose: 2000,
...
<InputCmp name="password" type='password' placeholder='********' divClassNames='w-full' onChange={handleInput} value={input.password} />
        </div>
        <div className='flex gap-4'>
<InputCmp name="position" placeholder='Poste' divClassNames='w-full' onChange={handleInput} value={input.position} />
<input type='date' name="startDate" placeholder='Date début' className='w-full py-2' onChange={handleInput} value={input.startDate} />
        </div>
        <div className='flex gap-4'>
